...
def connect_db(): #taken from text
    return sqlite3.connect(app.config['DATABASE'])

def init_db(): # taken from txt - creating a schema
...
def login(): #taken from text - login function
    error = None
...
@app.route('/logout')
def logout(): #taken from text - logout function
    session.pop('username', None)
    flash('You have been logged out')
    return redirect(url_for('show_quiz'))

@app.route('/dashboard') #taken from text - the view function
def show_quiz():
...
def add_student(): #taken from text - adding new entries
    if request.method == 'POST':
...
def show_results(id): #taken from text - the view function
    msg = None
    cur = g.db.execute(
...
def add_quiz(): #taken from text - adding new entries
    if request.method == 'POST':
        if not session.get('logged_in'):
            abort(401)
